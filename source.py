...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.tvguide.dvr] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.tvguide.dvr] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.tvguide.dvr] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.tvguide.dvr] Updating caches...', xbmc.LOGDEBUG)
    ...
    def exportChannelList(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/script.tvguide.dvr/channels.ini','wb')
    ...
    def _createTables(self):
        ...
        #except sqlite3.OperationalError, ex:
            ...
            xbmc.log("(script.tvguide.dvr) %s" % detail, xbmc.LOGERROR)
            dialog = xbmcgui.Dialog()
            dialog.notification('script.tvguide.dvr', 'database exception %s' % detail, xbmcgui.NOTIFICATION_ERROR , 5000)
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvguide.dvr'))
    KEY = 'xmltv'
    INI_TYPE_FILE = 0
    INI_TYPE_URL = 1
    INI_FILE = 'addons.ini'
    ...
    def __init__(self, addon):
        ...
        if self.xmltvType == XMLTVSource.XMLTV_SOURCE_FILE:
            ...
            if os.path.exists(customFile):
                ...
                xbmc.log('[script.tvguide.dvr] Use local file: %s' % customFile, xbmc.LOGDEBUG)
                self.xmltvFile = customFile
            else:
                # Probably a remote file
                xbmc.log('[script.tvguide.dvr] Use remote file: %s' % customFile, xbmc.LOGDEBUG)
        ...
        path = "special://profile/addon_data/script.tvguide.dvr/addons.ini"
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            try:
                ...
                xbmc.log('[script.tvguide.dvr] strptime error with this date: %s' % dateString, xbmc.LOGDEBUG)
