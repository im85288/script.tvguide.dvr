...
if __name__ == '__main__':
    try:
        ...
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(script.tvguide.dvr)")

        if ADDON.getSetting('background.service') == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[script.tvguide.dvr] Background service started...", xbmc.LOGDEBUG)
            Service()
            if ADDON.getSetting('service.addon.folders') == "true":
                xbmc.executebuiltin('RunScript(special://home/addons/script.tvguide.dvr/ReloadAddonFolders.py)')
            ...
            while not monitor.abortRequested():
                ...
                xbmc.log("[script.tvguide.dvr] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                if monitor.waitForAbort(waitTime):
                    # Abort was requested while waiting. We should exit
                    break
                xbmc.log("[script.tvguide.dvr] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/script.tvguide.dvr/ReloadAddonFolders.py)')
        ...
        xbmc.log('[script.tvguide.dvr] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)
