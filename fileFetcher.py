...
class FileFetcher(object):
    ...
    basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvguide.dvr'))
    filePath = ''
    fileUrl = ''
    addon = None
    ...
    def fetchFile(self):
        ...
        if fetch:
            ...
            if self.fileType == self.TYPE_DEFAULT:
                xbmc.log('[script.tvguide.dvr] file is in remote location: %s' % self.fileUrl, xbmc.LOGDEBUG)
                if not xbmcvfs.copy(self.fileUrl, tmpFile):
                    xbmc.log('[script.tvguide.dvr] Remote file couldn\'t be copied: %s' % self.fileUrl, xbmc.LOGERROR)
            else:
                f = open(tmpFile, 'wb')
                xbmc.log('[script.tvguide.dvr] file is on the internet: %s' % self.fileUrl, xbmc.LOGDEBUG)
            ...
            xbmc.log('[script.tvguide.dvr] file %s was downloaded' % self.filePath, xbmc.LOGDEBUG)
        return retVal